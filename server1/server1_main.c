...
#define TIMEOUT 15        //Timeout for waiting client messages before closing connection
#define SEND_TIMEOUT 10   //Timeout for waiting to send data to client
#define BUF_SIZE 1024
#define SMALL_BUF_SIZE 1024
#define SEND_BUF_SIZE 8192   //Sending buffer size
#define MSG_FILE 1
#define MSG_QUIT 0
#define MSG_UNKNOWN -1
#define BACKLOG 5         //Max number of clients waiting to be served
...
/*SEQUENTIAL SERVER: only a client at a time must be server. Server waits for GET messages and sends only files that are located
  in its working directory or in a lower location. Request for files in upper directories in respect to the working directory 
  are refused. Absolute paths are also refused. A timeout system has been implemented for the reception of client messages (so that
  other waiting client don't need to wait for too long), and also for sending messages to the client (so that if the transmission
  buffer is full because the client is not downloading, the connection will be closed after a while). If the server notices that 
  unknown messages are receieved, it sends back an ERR message and closes the connection.
  This server accepts both Ipv4 and Ipv6 connections.*/
...
int main(int argc, char **argv)
{
  ...
  //Creating the server's Socket: Ipv6 socket that also accepts Ipv4 connections
  ...
  while(1)
      ...
      else
	{
	  ...
	  printf("Accepted conn on socket %d from %s at port %d\n", clientSocket, incomingAddr, ntohs(clientSockaddr.sin6_port));
	  printf("\n");

	  //The server executes its duty
...
void serverService(int mySocket)
{
  ...
  while(1)
      ...
      if(result > 0)
	{
	  ...
	  receivingBuffer = calloc(sizeof(char), BUF_SIZE);
	  ...
	  if(rec > 0)
		...
		case MSG_FILE:
		  ...
		  if(filePath == NULL)
			...
			{
			  fprintf(stderr, "Could not send file. Closing connection with client\n");
			  free(receivingBuffer);
			  free(filePath);
			  close(mySocket);
		          return;
			...
			printf("-ERR message sent to the client due to FILE NOT FOUND or PERMISSION DENIED.\nClosing connection.\n");
		      free(receivingBuffer);
                      free(filePath);
		      close(mySocket);
		      return;
		...
		case MSG_QUIT:
		  printf("QUIT message received. Proceeding to close the connection...\n");
		  free(receivingBuffer);
		  close(mySocket);
		  return;
		...
		case MSG_UNKNOWN:
		  if(sendMessage(mySocket, "-ERR\r\n", 6) == 0)
		    printf("-ERR message sent to the client due to UNKNOWN COMMAND. Closing connection.\n");
		  free(receivingBuffer);
		  close(mySocket);
		  return;
	    ...
	    {
	      //Case of errors
	      fprintf(stderr, "Error while reciving message\n");
	      free(receivingBuffer);
	      close(mySocket);
	      return;
	    }
      ...
      else if(result == 0)
	{
	  ...
	  fprintf(stderr,"TIMEOUT: client on socket %d did not reply within %d seconds\n",mySocket,TIMEOUT);
	  close(mySocket);
	  return;
	}
      else
...
//This function checks if a file exists or if it is allowed to send the specific file. In case of success it returns 0. Otherwise -1.
int checkFileExistence(char *fileName, char *filePath)
{
  ...
  while (token) {
	...
	if(up > down)
	  {
	    fprintf(stderr,"Warning! Trying to access above server's working directory!\n");
	    free(tmpName);
	    return -1;
	  }
      }
...
int sendFile(int mySocket, char *fileName)
{
  ...
  if(!S_ISREG(fileStat.st_mode))
    {
      if(sendMessage(mySocket, "-ERR\r\n", 6) == 0)
	printf("-ERR message sent to the client due to FILE REQUESTED IS NOT A REGULAR FILE. Closing connection.\n");
      close(fd);
      return -1;
    }
  ...
  printf("File size is %u and timestamp is %u\n",fileSize_h,fileTime_h);
...
//This function sends with a timeout, in case of full transmission buffer
int sendWithTimeout(int mySocket, const void * buffer, size_t buflen, int flags)
{
  size_t nleft = buflen;
  ssize_t nwritten;
  ...
  while(nleft > 0)
      ...
      if( result > 0)
	{
	  ...
	  if(nwritten > 0)
	    ...
	    {
	      //An error was received
	      fprintf(stderr,"Error during send\n");
	      return -1;
	    }
	}
      else if(result == 0)
	{
	  //If we're here, TIMEOUT has expired
	  fprintf(stderr,"TIMEOUT has expired during send!\n");
	  return -2;
	}
      else
	{
